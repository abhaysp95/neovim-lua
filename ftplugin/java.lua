...
if vim.fn.has "mac" == 1 then
  WORKSPACE_PATH = home .. "/java_workspace/"
  CONFIG = "mac"
...
JAVA_DAP_ACTIVE = false  -- make this true (after setup)

--[[ local bundles = {
	vim.fn.glob(
		home .. "/.config/nvim/java-debug/com.microsoft."
	)
} ]]
...
local config = {
  ...
  capabilities = require("lsp.handlers").capabilites,
...
vim.cmd "command! -buffer -nargs=? -complete=custom,v:lua.require'jdtls'._complete_compile JdtCompile lua require('jdtls').com pile(<f-args>)"
vim.cmd "command! -buffer -nargs=? -complete=custom,v:lua.require'jdtls'._complete_set_runtime JdtSetRuntime lua require('jdtl s').set_runtime(<f-args>)"
